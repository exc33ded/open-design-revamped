...
import { injectSnapshotBridge } from '@open-design/contracts/runtime/preview-snapshot-bridge';
...
function injectUrlPreviewBridge(html: string, bridge: 'scroll' | 'selection' | 'snapshot'): string {
  ...
  // Shared with the srcDoc pipeline and the powered-preview route (see
  // packages/contracts/src/runtime/preview-snapshot-bridge.ts) — was
  // previously a hand-duplicated copy here that had drifted out of sync with
  // the tainted/cross-origin image handling added to the srcDoc version.
  return injectSnapshotBridge(html);
}
    ...
    try {
      ...
      await sendProjectFile(
        ...
        skipPoweredTransform ? undefined : async (file) => {
          const resolved = await maybeResolveVitePreviewHtml({
            file,
            projectId,
            relPath,
            metadata: project?.metadata,
          ...
          // Give the live, URL-loaded document its own `od:snapshot` listener
          // so Mark/Screenshot can capture it over postMessage without the
          // web host remounting it into a srcDoc iframe — that remount is a
          // second page load that breaks a powered preview's own backend
          // fetches (see FileViewer's srcDocForcedOnlyByDraw).
          if (!/^text\/html(?:;|$)/i.test(file.mime)) return resolved;
          const html = Buffer.isBuffer(resolved) ? resolved.toString('utf8') : resolved;
          return injectSnapshotBridge(html);
        },
      );
    } catch (err: any) {
